"""
视频去水印工具 - Kivy Android 版 (v2 - 无 OpenCV 依赖)
使用 ffmpeg 命令行处理视频，避免 OpenCV 编译问题
"""
import os
import subprocess
import threading
...
class WatermarkRemoverApp(App):
    def build(self):
        ...
        title = Label(text='视频去水印工具', font_size=24, size_hint_y=0.1, bold=True)
        layout.add_widget(title)

        self.status_label = Label(text='正在检查环境...', font_size=16, size_hint_y=0.1)
        layout.add_widget(self.status_label)

        select_btn = Button(text='选择视频文件', size_hint_y=0.1, background_color=(0.2, 0.6, 1, 1))
        select_btn.bind(on_press=self.show_file_chooser)
        layout.add_widget(select_btn)

        self.file_label = Label(text='未选择文件', font_size=14, size_hint_y=0.1, color=(0.5, 0.5, 0.5, 1))
        layout.add_widget(self.file_label)

        self.progress = ProgressBar(max=100, size_hint_y=0.05, value=0)
        layout.add_widget(self.progress)

        self.process_btn = Button(text='开始处理', size_hint_y=0.1, background_color=(0.2, 0.8, 0.2, 1), disabled=True)
        self.process_btn.bind(on_press=self.process_video)
        layout.add_widget(self.process_btn)

        footer = Label(text='提示：处理后的视频保存在原文件同目录', font_size=12, size_hint_y=0.1, color=(0.6, 0.6, 0.6, 1))
    ...
    def _check_ffmpeg(self, dt):
        ...
        if self.ffmpeg_cmd:
            ...
            self.status_label.text = '⚠ 未找到 ffmpeg，将使用基础裁剪模式'
    ...
    def _process_with_ffmpeg(self, output_path):
        ...
        # 获取视频信息
        probe_cmd = [
            self.ffmpeg_cmd, '-i', self.selected_file,
            '-hide_banner'
        ]
        result = subprocess.run(probe_cmd, capture_output=True, text=True, timeout=30)

        # 裁剪右下角 10% 水印区域
        # crop=in_w*0.9:in_h*0.9:0:0 保留左上 90% 区域
        crop_filter = "crop=iw*0.9:ih*0.9:0:0"
        ...
        Clock.schedule_once(lambda dt: self._update_status('正在用 ffmpeg 处理...'), 0)
        ...
        if process.returncode == 0:
            ...
            # ffmpeg 失败，回退到基础模式
            Clock.schedule_once(lambda dt: self._update_status('ffmpeg 处理失败，回退到基础模式...'), 0)
            self._process_basic_copy(output_path)
    ...
    def _process_basic_copy(self, output_path):
        ...
        Clock.schedule_once(lambda dt: self._update_status(
            f'已复制到: {os.path.basename(output_path)}\n'
            '注意：基础模式不处理水印，请安装 ffmpeg 后重试'
        ), 0)
